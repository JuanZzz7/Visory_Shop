@extends('layouts.app')
@section('title','Iniciar Sesión - Spotlight')
@section('content')
<div class="min-vh-100 d-flex align-items-center justify-content-center" style="background-color: var(--vs-bg-color);">
    <div class="vs-card border-0" style="width: 100%; max-width: 900px; border-radius: var(--vs-radius-xl); overflow: hidden; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.15);">
        <div class="row g-0">
            ...
            <div class="col-md-5 d-none d-md-block overflow-hidden" style="background: radial-gradient(circle, rgb(10 60 202) 0%, #042e52 100%); position: relative;">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" preserveAspectRatio="none" class="login-bg-svg position-absolute top-0 start-0 w-100 h-100" style="opacity: 0; z-index: 0;">
                  <defs />
                ...
                <div class="position-relative d-flex flex-column justify-content-between h-100 p-5 text-white" style="z-index: 2;">
                    ...
                    <div class="mt-auto">
                        <h2 class="fw-bold mb-3 text-white">La plataforma ideal para tu negocio.</h2>
                        <p class="text-white" style="opacity: 0.9;">Accede a tu cuenta para gestionar tus productos, compras y operaciones desde un solo lugar.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-7 p-4 p-md-5 bg-white">
                ...
                <div class="mb-4">
                    ...
                    <p class="text-muted">Ingresa tus credenciales para acceder a tu panel.</p>
                </div>
                ...
                <form action="{{ route('login') }}" method="POST">
                    ...
                    <div class="mb-4">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <label class="form-label text-sm fw-semibold text-dark mb-0">Contraseña</label>
                            <a href="#" class="text-primary text-decoration-none text-sm fw-medium">¿Olvidaste tu contraseña?</a>
                        </div>
                        <input type="password" name="password" class="form-control form-control-lg text-sm" placeholder="••••••••" required>
                    </div>
                    
                    <div class="mb-5">
                        <label class="form-label text-sm fw-semibold text-dark mb-3">Selecciona tu rol</label>
                        <div class="row g-2">
                            @foreach(['user'=>'Usuario','business'=>'Empresario'] as $val=>$label)
                            <div class="col-6">
                                <input type="radio" class="btn-check" name="role" id="role_{{ $val }}" value="{{ $val }}" {{ old('role')===$val || (!old('role') && $val==='user') ? 'checked' : '' }}>
                                <label class="btn btn-outline-primary w-100 text-sm py-2 px-1 fw-medium" for="role_{{ $val }}">{{ $label }}</label>
                            </div>
                            @endforeach
                        </div>
                    </div>
                    
                    <button type="submit" class="btn btn-primary btn-lg w-100 fw-semibold shadow-sm mb-4">Iniciar sesión</button>

                    <div class="position-relative mb-4 text-center">
                        <hr class="text-muted opacity-25">
                        <span class="position-absolute top-50 start-50 translate-middle bg-white px-3 text-muted text-sm fw-medium">O continuar con</span>
                    </div>
                    ...
                    <div class="text-center">
                        <p class="text-muted text-sm mb-0">¿No tienes cuenta? <a href="{{ route('register') }}" class="text-primary fw-semibold text-decoration-none">Regístrate gratis</a></p>
                    </div>
                </form>
            </div>
        </div>
